...
masha_cmd = "tail -n 1 /tmp/masha.log | awk -F'/' '{print $6}' | sed 's/\..\{3\}$//' | tr -d '\n'"
barba_cmd = "tail -n 1 /tmp/barba.log | awk -F'/' '{print $6}' | sed 's/\..\{3\}$//' | tr -d '\n'"
peppa_cmd = "tail -n 1 /tmp/peppa.log | awk -F'/' '{print $6}' | sed 's/\..\{3\}$//' | tr -d '\n'"
...
pending_cmd = "atq | cut -d ' ' -f2-4 | cut -d ':' -f1-2 | tr -d '\n'"
...
def playm_video():
    ...
    while(1):
     ...
     for i in range (0, len(my_title)):
      ...
      mylcd.lcd_display_string(chr(4) + " " + chr(4) + " " + lcd_status + " " + chr(4) + " " + chr(4) + " ",1)
      time.sleep(0.1)
      f = os.popen("cat /tmp/statusm | tr -d '\n'")
      time.sleep(0.2)
	 ...
	 lcd_status = "PAUSED"
         ...
         masha_menu()
      if ( GPIO.input(UP) == False):
       os.system("/home/pi/scripts/VolUpm.sh")
      if ( GPIO.input(DOWN) == False):
...
def playb_video():
    ...
    while(1):
     ...
     for i in range (0, len(my_title)):
      ...
      mylcd.lcd_display_string(chr(4) + " " + chr(4) + " " + lcd_status + " " + chr(4) + " " + chr(4) + " ",1)
      time.sleep(0.1)
      f = os.popen("cat /tmp/statusb | tr -d '\n'")
      time.sleep(0.2)
      for i in f.readlines():
        status = int(i)
        if status == 1:
...
def playp_video():
    ...
    while(1):
     ...
     for i in range (0, len(my_title)):
      ...
      mylcd.lcd_display_string(chr(4) + " " + chr(4) + " " + lcd_status + " " + chr(4) + " " + chr(4) + " ",1)
      time.sleep(0.1)
      f = os.popen("cat /tmp/statusp | tr -d '\n'")
      time.sleep(0.2)
      for i in f.readlines():
        status = int(i)
        if status == 1:
...
def get_pending():
    pending = subprocess.check_output(pending_cmd, shell=True, stderr=subprocess.STDOUT)
    return pending
...
def get_masha_title():
    p = Popen(masha_cmd, shell=True, stdout=PIPE)
    masha = p.communicate()[0]
    return masha

def get_barba_title():
    p = Popen(barba_cmd, shell=True, stdout=PIPE)
    barba = p.communicate()[0]
    return barba

def get_peppa_title():
    p = Popen(peppa_cmd, shell=True, stdout=PIPE)
    peppa = p.communicate()[0]
    return peppa
